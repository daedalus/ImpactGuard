...
def _unwrap_value(raw: str) -> Any:
    ...
    if not isinstance(wrapped, dict) or "_t" not in wrapped:
        return wrapped
    if wrapped["_t"] == "ast_dump":
        import ast as _ast
        return _ast.parse(wrapped["v"])
...
class Cache:
    ...
    def _init_bloom(self) -> None:
        ...
        try:
            ...
            logger.debug("Cache bloom init skipped due to locked database")
    ...
    def get(self, key: str) -> Any:
        ...
        try:
            ...
            logger.debug("Cache get skipped due to locked database: %s", canonical_key)
        _record_cache_miss(canonical_key)
        return None
